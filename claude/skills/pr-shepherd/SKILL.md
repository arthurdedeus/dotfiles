---
name: pr-shepherd
description: Use when you want to drive a PR through the review loop autonomously — running qa-swarm, triaging qa-swarm and bot review comments, keeping the branch current with its base, watching CI, and re-applying the `stamphog` label whenever the actionable-thread state is clean. One invocation runs a single iteration; wrap in `/loop` for hands-off cadence. Defers anything ambiguous to the user.
argument-hint: [pr-number-or-url] [state...]
---
